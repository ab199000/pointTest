...
window.onload = function() {
	...
	document.querySelector('.box').on('click', function(event) {

        let targetElem = event.target
		
		if(targetElem.hasClass('hide')) {			
			var box = targetElem.attr('id').split('-');
			...
			if(invalidTestBoard() || invalidMove(x, y)) {
				...
				targetElem.addClass(player === 0 ? 'black' : 'white');
	...
	document.querySelector('.undo-btn').on('click', function() {		
	...
	document.querySelector('.reset-btn').on('click', function() {
		reset();		
		paintBoard();
	});
...
function updateGameBy(board) {
	deleteBox(document.querySelector('.box'));
	
	for(var i = 0; i < boardSize; i++) {
		for(var j = 0; j < boardSize; j++) {
			if(board[i][j] !== EMPTY) {
				var box = doqument.createElement('#box-' + i + '-' + j);
...
function showInvalidMove(x, y) {
	var box = document.querySelector('#box-' + x + '-' + y);
...
function createGrid() {
	// document.querySelector('.board').empty();
	...
	for(var i = 0; i < boardSize; i++) {
		...
		for(var j = 0; j < boardSize; j++) {
			...
			('#row-' + i).innerHTML +'<div class=\"box hide\" id=\"box-' + i + '-' + j + '\">';
		}
	}
}
